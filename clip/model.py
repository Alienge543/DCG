...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(
        ...
        self.compound_prompt_nvtx = design_details["maple_length_v"]
        if i == 0:
    ...
    def attention(self, x: torch.Tensor):
        ...
        out,atten = self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)
    ...
    def forward(self, inputs):
        ...
        if not self.first_layer:
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
#                     x = x.unsqueeze(0).expand(shared_ctx.shape[0],-1,-1,-1)
#                     visual_ctx = shared_ctx.unsqueeze(1).expand(-1,x.shape[1],-1,-1)
...
#                     x = x.contiguous().view(x.shape[0]*x.shape[1], x.shape[2], x.shape[3])
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        M,n_vctx,dim = visual_context.shape
                        L,MB,_ = x.shape
                        # Remove the outputs produced by learnable tokens of previous layer
                        
                        x = x.contiguous().view(-1, M,MB//M,dim)
                        
                        prefix = x[0 : x.shape[0] - self.compound_prompt_nvtx, :, :,:]  #torch.Size([197, 4, 128, 768]) torch.Size([199, 4, 128, 768]) torch.Size([4, 2, 768])\

                        visual_context = (
                            visual_context.unsqueeze(1).expand(-1,x.shape[2], -1,-1)    
                            .permute(2, 0, 1,3)
                            .half()
                        )
                       
                        # torch.Size([4, 128, 2, 768])
                        x = torch.cat([prefix, visual_context], dim=0).reshape(L,MB,dim)
...
#                         x = torch.cat([prefix, visual_context], dim=0)
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        n,n_ctx,dim = textual_context.shape
                        if textual_context.dim() == 3:
                            textual_context = textual_context.unsqueeze(0).expand(x.shape[1]//n, -1, -1,-1)
                        textual_context = textual_context.permute(1, 0, 2, 3)
                        textual_context = textual_context.contiguous().view(x.shape[1],n_ctx,dim).permute(1, 0, 2).half()
#                         print("================================>text encoder2")#torch.Size([4, 76, 512])torch.Size([1, 76, 512])
#                         print(self.compound_prompt_nctx)
...
#         x = x + self.positional_embedding.to(x.dtype)  #128, 197, 768
# #         print("==========================================>visual encoder")
# #         print(x.shape)
#         # After positional embeddings, we will attach prompts with the model, remember only those
#         # are trainable parameters here in whole image encoder.
#         if prompt.dim() >= 3: #[4, 2, 768]
...
def compute_similarity(feature1, feature2):
    return F.cosine_similarity(feature1.unsqueeze(0), feature2.unsqueeze(0))
